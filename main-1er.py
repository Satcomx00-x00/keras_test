...
x_train = np.fliplr(np.rot90(x_train, k=-1, axes=(1, 2)).reshape(x_train.shape[0], 28, 28, 1))
x_test = np.fliplr(np.rot90(x_test, k=-1, axes=(1, 2)).reshape(x_test.shape[0], 28, 28, 1))
...
model.add(tf.keras.layers.Dense(units=50,input_shape=(784,),activation='relu'))
